...
def check_image(image_s3_path, endpoint_name):
    X = []
    image_temp_path = "temp/"+image_s3_path.split("/")[-1]
    s3.download_file(
        Bucket = image_s3_path.split("/")[2],
        Key = "/".join(image_s3_path.split("/")[3:]),
        Filename = image_temp_path
    )
    X.append(np.array(convert_to_ela_image(image_temp_path, 90).resize((128, 128))).flatten() / 255.0)
    X = np.array(X)
